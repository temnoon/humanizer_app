...
const MaieuticDialogue = ({ narrative, onComplete, isActive }) => {
  ...
  const startSession = async () => {
    ...
    setIsLoading(true)
    try {
      const response = await fetch('/api/maieutic/start', {
        method: 'POST',
  ...
  const submitAnswer = async () => {
    ...
    setIsLoading(true)
    try {
      const response = await fetch('/api/maieutic/answer', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
  ...
  const completeSession = async () => {
    ...
    setIsLoading(true)
    try {
      const response = await fetch(`/api/maieutic/complete?session_id=${sessionId}`, {
        method: 'POST'
      })
